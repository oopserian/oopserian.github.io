...
export const ProjectCard: React.FC<ProjectCardProps> = ({ logo, name, description, link }) => {
    ...
    return (
        <div onClick={jumpLink} className={cn(
            "group relative overflow-hidden flex gap-2 items-center p-3 border border-zinc-800 rounded-2xl select-none",
            (link ? "cursor-pointer" : "")
        )}>
            <img className="absolute h-full w-auto top-0 -left-2 blur-lg opacity-0 transition-opacity group-hover:opacity-50" src={logo} />
